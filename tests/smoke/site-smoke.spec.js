...
test.describe('repo smoke flows', () => {
  test('homepage exposes the main spiritual archive entry points', async ({ page }) => {
    await gotoAndStabilize(page, '/');

    const articlesNavLink = page.locator('header').getByRole('link', { name: /^Articles$/ });

    await expect(page.getByRole('heading', { name: /a flower at the guru's feet/i })).toBeVisible();
  ...
  test('search flow leads from the homepage into a matching article', async ({ page }) => {
    await gotoAndStabilize(page, '/');

    const homepageSearch = page.locator('form[action="/search/"] input[name="q"]');
    await homepageSearch.fill('bhairava');
    await homepageSearch.press('Enter');

    await expect(page).toHaveURL(/\/search\/\?q=bhairava/i);
    await expect(page.getByRole('heading', { name: /search all articles/i })).toBeVisible();
  ...
  test('gallery lightbox opens, advances, and closes for sacred art', async ({ page }) => {
    await gotoAndStabilize(page, '/gallery.html');

    await expect(page.getByRole('heading', { name: /sacred art gallery/i })).toBeVisible();
    ...
    await expect(page.locator('#lightbox-caption')).toContainText(/Jai_Ma_|shmashana|bhairava|kali|kalika/i);
